  ...
  getHV() {
    ...
    for (let word of palabras) {
      ...
      for (let p of this.listaH) {
        if (p.name.toUpperCase() === word.toUpperCase()) repetida = true;
      }

      if (prepos.includes(word.toLowerCase())) repetida = true;
      if (abc.includes(word.toLowerCase())) repetida = true;
      if (word == "") repetida = true;
      if (dets.includes(word.toLowerCase())) repetida = true;
      if (random.includes(word.toLowerCase())) repetida = true;
      if (adjs.includes(word.toLowerCase())) repetida = true;
      if (eng.includes(word.toLowerCase())) repetida = true;
...
*/
    ...
    */
    ...
    let barras = chart.selectAll()
      ...
      .attr('fill', (d) => {
        if(this.aSeleccionados.indexOf(d.ano)) return '#00ffff'
        else return '#00af05'
      })
      .attr('stroke', 'blue')
      .attr('id', (s) => s.ano)
...
*/
    ...
    let circles = node
      ...
      .attr('class', 'bbub')
      .attr('id', (d) => d.name)
      .attr('r', function(d) { return d.value / media * 25 })
      //.attr('r', function(d) { return d.value * factor / 200 })
      .attr('stroke', 'blue')
      .attr('fill', (d) => {
        if(this.tSeleccionados.indexOf(d.name)) return '#00ffff'
        else return '#00af05'
      });
